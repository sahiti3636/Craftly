...
inference featurise a product with exactly the same code.
...
`python data.py verify preprocess`:
...
```bash
python data.py                 # download → verify → clean → split
python features.py             # images + CLIP + MiniLM + structured features
python train.py                # tune, train, select, calibrate, evaluate, write MODEL_CARD.md
python service.py selftest     # 10 contract checks — exit 0 = safe to lock in
pytest -q test.py              # 38 tests
python service.py demo         # end-to-end demo on a held-out real product
python service.py serve        # API on :8000 (or: uvicorn service:app --port 8000)
```

Useful flags:

```bash
python data.py --skip-amazon                 # Flipkart only
python data.py verify preprocess             # after a manual download
python features.py --no-images               # skip the image download
python features.py --download-encoders       # fetch encoder weights once, with progress
python train.py --select-by RMSE             # different selection metric
python train.py --no-tune                    # skip the hyper-parameter grid
python service.py demo --image my.jpg --title "..." --category home_decor \
       --material brass --material-cost 400 --labour-hours 10
python service.py serve --device cpu --port 8000
```
...
Excerpt from `python service.py demo` — a held-out **test-split** product the model never trained on:
...
```bash
pytest -q test.py              # 38 tests
python service.py selftest     # 10 contract checks on the trained artifacts
```
